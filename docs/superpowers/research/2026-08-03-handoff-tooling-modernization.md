...
- `docs/superpowers/research/2026-08-03-minified-line-node-count.md` — a single 20,000-character line
  renders **40,058 DOM nodes** and takes 11s, against 1,203 nodes and 0.4s for the whole 20,000-*line*
...
per-file spawn loop with every status hardcoded to `"modified"`. It now means *the working tree
versus where this branch diverged from `<ref>`*, reusing the fast whole-tree path. Merge base rather
